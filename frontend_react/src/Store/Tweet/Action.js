...
export const updateMeal = (reqData, twitId) => async (dispatch) => {
  console.log('Tweet Data', reqData);
  dispatch({ type: UPDATE_TWEET_REQUEST });
  try {
    const response = await api.put(`/api/twits/${twitId}/mealupdate`, reqData);
    const tweet = response.data;
    console.log('updated tweet -: ', tweet);

    dispatch({ type: UPDATE_TWEET_SUCCESS, payload:tweet });
  } catch (error) {
